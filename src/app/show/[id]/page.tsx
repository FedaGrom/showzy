...
export default async function ShowDetails({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <p className="mb-2">Ocjena: {show.rating.average ?? "N/A"}</p>
      <p className="mb-2">Žanrovi: {show.genres.join(", ")}</p>
      ...
      <h2 className="text-2xl font-semibold mt-8 mb-4">Glumci</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {cast.map((member) => (
          <div key={member.person.id} className="text-center">
            ...
            <p className="text-sm text-gray-400">kao {member.character.name}</p>
          </div>
        ))}
      </div>
    </div>
